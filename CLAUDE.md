...
- **Changelog**: the root `CHANGELOG.md` is generated and is not edited by hand -- a repository-level change writes its own fragment under `.changes/unreleased/` with `chlog new --kind <Kind> --body '...'` (see the chlog section below); `mcp-server-extended/` keeps its own nested `CHANGELOG.md` for the Go add-on.
